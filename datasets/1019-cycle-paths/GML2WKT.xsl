...
<xsl:stylesheet version="2.0"
    ...
    <xsl:variable name='epsg'
                  select="concat('http://www.opengis.net/def/crs/EPSG/','0/')" />
    ...
    <xsl:template match="gml:boundedBy">
        <xsl:apply-templates />
        <!-- May involve either a gml:Box (older specification) or a gml:Envelope (latest OGC standard) -->
    </xsl:template>
    <xsl:template match="gml:Box">
        ...
        <xsl:variable name="coords"
                      select="tokenize(replace(replace(gml:coordinates,'\s+$',''),'^\s+',''), '[,\s\n\r]+')" />
    ...
    <xsl:template match="gml:Envelope">
        ...
        <xsl:call-template name="gml:srsName" />
        <xsl:variable name="MINX">
            <xsl:value-of select="normalize-space(substring-before(gml:lowerCorner, $separator))" />
        </xsl:variable>
        <xsl:variable name="MINY">
            <xsl:value-of select="normalize-space(substring-after(gml:lowerCorner, $separator))" />
        </xsl:variable>
        <xsl:variable name="MAXX">
            <xsl:value-of select="normalize-space(substring-before(gml:upperCorner, $separator))" />
        </xsl:variable>
        <xsl:variable name="MAXY">
            <xsl:value-of select="normalize-space(substring-after(gml:upperCorner, $separator))" />
        </xsl:variable>
        <xsl:value-of select="concat('POLYGON((', $MINX, ' ', $MINY, ', ', $MAXX, ' ', $MINY, ', ', $MAXX, ' ', $MAXY, ', ', $MINX, ' ', $MAXY, ', ', $MINX, ' ', $MINY, '))')" />
    </xsl:template>
